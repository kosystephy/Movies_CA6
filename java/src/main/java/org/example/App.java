...
public class App
{
	...
	{
		MovieDaoInterface movieDao = new MySqlMovieDao();  //"IUserDao" -> "I" stands for for
...
//        // only references of the interface type to access the DAO methods.
		...
		{
			System.out.println("\nCall findAllMovies()");
			List<Movie> users = IUserDao.findAllUsers();     // call a method in the DAO
			...
			users = IUserDao.findAllUsersLastNameContains("Sm");     // call a method in the DAO

			if( users.isEmpty() )
				System.out.println("There are no Users");
			else {
				for (User user : users)
					System.out.println("User with last name like Sm: " + user.toString());
			}

			System.out.println("\nCall addUser()");
			User user1=new User("Kosy","Ike","koko","password");
			users = IUserDao.addUser(user1);     // call a method in the DAO

			if( users.isEmpty() )
				System.out.println("There are no Users");
			else {
				for (User user : users)
					System.out.println("the user1 was added: " + user.toString());
			}
